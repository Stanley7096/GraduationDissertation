...
model.add(Dropout(0.1))
model.add(LSTM(256,return_sequences=False))  # 三层lstm，可调参数为240,256这个隐藏层单元以及lstm的层数
model.add(Dropout(0.1))
model.add(Dense(y_train.shape[1]))
model.add(Activation("tanh"))
model.compile(loss='mse', optimizer='adam', metrics=['acc'])#可调loss的函数，优化器optimizer的函数（keras官网有说明可以用哪些）

# 训练网络，可以更改训练的次数epochs，每轮取数据的大小batch_size
